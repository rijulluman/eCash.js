  ...
  addUnconfirmedTransaction: function(transaction, callback) {
  	...
  	async.waterfall([
  			...
  			function setTransaction(currentBlock, cb){
  				...
  				RedisStoreMA.setex(redisPath.unconfirmedTransaction + transaction.txId, ttl, JSON.stringify(transaction) , cb);
  			},
  			function sortTransactionByFee(cb){
  				RedisStoreMA.zadd([redisPath.sortedUnconfirmedTransaction, transaction.fees, transaction.txId], cb);
  			},
  		], 
